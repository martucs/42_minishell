...
/*   Updated: 2024/12/18 20:45:42 by martalop         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	add_node_without_val(t_envp *tmp, char *export)
{
	t_envp	*new;
...
int	copy_without_equal(t_envp *tmp, char *export, int i)
{
	free(tmp->key);
	free(tmp->value);
...
int	copy_with_eq(t_envp *tmp, char *export, int i)
{
	free(tmp->value);
	if (export[ft_strlen(tmp->key)] != 0)
		tmp->value = ft_substr(export, ft_strlen(tmp->key), \
				ft_strlen(export) - i);
	else
	{
		tmp->value = malloc(sizeof(char));
